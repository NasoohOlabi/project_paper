...
\begin{enumerate}
    \item \textbf{Dictionary Synthesis:} A session-specific linguistic dictionary is synthesized from the post body, the flattened comment bodies, and the fetched search-results text corpus.
...
\begin{enumerate}
    ...
    \item Index 0 corresponds to a top-level reply to the post, while any non-zero index selects a comment whose parent chain is reconstructed through parent-identifier and link-identifier lookups.
\end{enumerate}
