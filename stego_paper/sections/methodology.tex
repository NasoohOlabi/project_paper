...
\begin{enumerate}
    ...
    \item The physical index width is computed from the available choices, but the lossless payload width is $\lfloor\log_2 C\rfloor$, where $C$ is the number of selectable reply targets. This excludes non-injective index encodings when the choice count is not a power of two.
...
\begin{enumerate}
    ...
    \item \textbf{Dynamic Bit-Mapping:} The flattened list is assigned sequential \texttt{idx} values. The lossless bit width is $\lfloor\log_2 A\rfloor$ for $A$ available Angles; unused, aliased bit patterns are not counted as recoverable capacity.
...
\begin{enumerate}
    ...
    \item \textbf{Discriminative Identification:} A deterministic identification prompt is executed on the shortlisted candidates to return the most likely Angle index:
          ...
          \end{quote}
\end{enumerate}
If the discriminator returns a 1-based rank rather than a raw index, the pipeline maps the rank back to the shortlist; otherwise it falls back to the top semantic match. The decode temperature and shortlist size are configurable, so this stage is not inherently deterministic. Recovery may be pure selection-channel recovery or audit-assisted when sender metadata is available. Combined with Layer~1 (reply targeting), the recovered indices reconstruct the selection bits.
\FloatBarrier
