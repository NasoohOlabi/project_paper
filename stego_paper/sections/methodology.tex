...
\begin{enumerate}
    \item \textbf{Angle Set Reconstruction:} Using the deterministic preprocessing chain (Section \ref{sec:search_determinism}) and the filtered thread snapshot, both parties regenerate the same ordered list of candidate Angles, each described by \texttt{category}, \texttt{source\_quote}, and a \texttt{tangent} field (the thematic direction).
    \item \textbf{Dynamic Bit-Mapping:} The bit-width for this layer is computed from the total number of Angles, and the flattened list is assigned sequential \texttt{idx} values.
